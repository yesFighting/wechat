...
function filterAsyncRouter(asyncRouterMap, menus){
    //返回满足条件的子路由对象
    const accessedRouters = asyncRouterMap.filter(route =>{   //数组过滤筛选的方法
...
const permission = {
    state: {
        routers: constantRouterMap,   //本用户所有的路由，包括了固定路由和下面的和下面的addRouters
    ...
    mutations: {
        SET_ROUTERS: (state, routers)=> {
            ...
            state.routers = constantRouterMap.concat(routers)  //总路由（将普通路由和权限路由合并，成为本用户最终的全部路由信息）
